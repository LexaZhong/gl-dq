...
| 🎯 Target analysis | *(Portfolio analysis)* pick a target — **frequency**, **severity**, **loss cost** or **loss ratio** — and see the one-way relativity of each rating variable (target by level, its weight and credibility, against the portfolio average), then which **pairs interact**: every pair ranked by how far it departs from what the two one-ways predict alone, with the interaction plot (parallel lines = independent) and a lift heatmap of actual ÷ main-effects expected. One `GROUPING SETS` query returns the total, every one-way and every two-way, so adding a variable costs no extra query |
