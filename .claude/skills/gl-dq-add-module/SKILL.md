...
- [ ] Findings carry the grouping levels in `segment` via `segment_key({dim: value})`; `status_table` splits that label back into one column per dimension (`gl_dq.core.results.split_segment_columns`), so a table you build yourself with `st.dataframe` should either call that helper or carry the dimension columns directly.
